...
///////////////////////////////////////////////////
BlockPrefix_t *bestFit(size_t s) {	/* find first block with usable space > s */
    int bestSize = -1;
    BlockPrefix_t *bestP = NULL;
    BlockPrefix_t *p = arenaBegin;
    while (p) {
      if (!p->allocated && computeUsableSpace(p) >= s){
	  int diffSpace = computeUsableSpace(p) - s;
	  if(bestSize == -1){
	    bestSize = diffSpace;
	    bestP = p;
	  }
